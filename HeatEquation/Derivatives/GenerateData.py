...
def generate(noise):
    ...
    bnn_state_dict = torch.load('/BayesianDeepLearningPDE/HeatEquation/SavedModels/BNN_Noise' + noise + '/bnn_sigma_prior' + str(sigma_prior) + '.pt')
    bnn.load_state_dict(bnn_state_dict)
    params_hmc = torch.load('/BayesianDeepLearningPDE/HeatEquation/SavedModels/BNN_Noise' + noise + '/params_hmc.pt')
    params_hmc = params_hmc[200:6000]

    dnn = DeepNeuralNetwork(n_hidden_units)
    dnn_state_dict = torch.load('/BayesianDeepLearningPDE/HeatEquation/SavedModels/DNN_Noise' + noise + '/dnn_lr_' + lr + '_iter_' + n_iter + '.pt')

    mx = np.load('/BayesianDeepLearningPDE/HeatEquation/SavedModels/BNN_Noise' + noise + '/mx.npy')
    sx = np.load('/BayesianDeepLearningPDE/HeatEquation/SavedModels/BNN_Noise' + noise + '/sx.npy')
    ...
    np.save('/BayesianDeepLearningPDE/HeatEquation/Derivatives/DerivativesData/BNN_Noise' + noise + '/Derivatives_BNN_' + str(generator.n_hmc_samples) + 's_' + str(n_derivatives) + 'pt.npy', Derivatives_bnn)
    np.save('/BayesianDeepLearningPDE/HeatEquation/Derivatives/DerivativesData/BNN_Noise' + noise + '/Derivatives_Coordinates_' + str(generator.n_hmc_samples) + 's_' + str(n_derivatives) + 'pt.npy', coord)

    print('Noise Case: ' + noise + ', BNN Derivatives: DONE')

    Derivatives_dnn = generator.get_derivatives_dnn(n_derivatives, coord = coord)
    np.save('/BayesianDeepLearningPDE/HeatEquation/Derivatives/DerivativesData/DNN_Noise' + noise + '/Derivatives_DNN_' + str(n_derivatives) + 'pt.npy', Derivatives_dnn)
    np.save('/BayesianDeepLearningPDE/HeatEquation/Derivatives/DerivativesData/DNN_Noise' + noise + '/Derivatives_Coordinates_' + str(n_derivatives) + 'pt.npy', coord)
